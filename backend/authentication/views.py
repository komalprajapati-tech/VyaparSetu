...
def register(request):
    try:
        ...
        # Check if user already exists in this module's database
            ...
            if user:
                if user.is_verified:
                    return Response(
                        {
                            "success": False,
                            "message": "Email already registered under this business type."
                        },
                        status=409
                    )
                else:
                    # If user is unverified, check rate limit and resend OTP
                    success, message = resend_otp_service(user.email, business_type=business_type)
                    if not success:
        ...
        # Register new user in module database
...
def verify_otp(request):
    ...
    with switch_db(User, alias), switch_db(OTP, alias):
...
def resend_otp_view(request):
    ...
    success, message = resend_otp_service(email, business_type=business_type)
    if not success:
        if "Too many OTP requests" in message:
            return Response(
                {
...
def login(request):
    ...
    requested_business_type = data.get("businessType")
    ...
    if requested_business_type:
        target_alias = get_db_alias(requested_business_type)
        with switch_db(User, target_alias):
            u = User.objects(email=email).first()
            if u:
                user = u
    else:
        # Search across all module databases
        for alias_name in ["retailer", "food", "service"]:
            with switch_db(User, alias_name):
                u = User.objects(email=email).first()
    ...
    if not user or not target_alias:
        return Response(
            {
                "success": False,
...
def forgot_password(request):
    ...
    success, message = forgot_password_service(email, business_type=business_type)
    if not success:
        if "Too many OTP requests" in message:
            return Response(
                {
...
def reset_password(request):
    ...
    with switch_db(User, alias), switch_db(ResetOTP, alias):
        user = User.objects(email=email).first()
        if not user:
            return Response(
                {
...
def user_profile(request):
    ...
    try:
        ...
        business_type = payload.get("business_type") or payload.get("businessType") or "retailer"
        alias = get_db_alias(business_type)
    except Exception:
...
def notifications_list(request):
    ...
    with switch_db(User, alias), switch_db(Notification, alias):
        user = User.objects(email=user_email).first()
        if not user:
            return Response({"success": False, "message": "User not found."}, status=404)
...
def mark_notification_read(request, notif_id):
    ...
    with switch_db(Notification, alias):
